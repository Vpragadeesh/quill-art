...
int main(int, char **) {
  ...
  init_info.PhysicalDevice = state.physical_device;
  init_info.Device = state.device;
  init_info.QueueFamily = state.queue_family;
  init_info.Queue = state.queue;
  init_info.PipelineCache = state.pipeline_cache;
  init_info.DescriptorPool = state.descriptor_pool;
  init_info.RenderPass = wd->RenderPass;
  init_info.Subpass = 0;
  init_info.MinImageCount = state.min_image_count;
  init_info.ImageCount = wd->ImageCount;
  init_info.MSAASamples = VK_SAMPLE_COUNT_1_BIT;
  init_info.Allocator = state.allocator;
  ...
  while (!done) {
    ...
    if (fb_width > 0 && fb_height > 0 &&
        (state.swapchain_rebuild || state.main_window_data.Width != fb_width ||
    ...
    if (!is_minimized) {
      ...
      state.render_frame(wd, draw_data);
      state.present_frame(wd);
    }
  }

  // Cleanup
  err = vkDeviceWaitIdle(state.device);
  check_vk_result(err);
  ImGui_ImplVulkan_Shutdown();
  ImGui_ImplSDL3_Shutdown();
  ImGui::DestroyContext();

  state.cleanup_vulkan_window();
  state.cleanup_vulkan();
